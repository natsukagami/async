...
class FutureBehavior extends munit.FunSuite {
  ...
  test("alt") {
    Async.blocking:
      ...
      assertEquals(fail.alt(fail1).awaitTry, Failure(error))
  }
  ...
  test("altC of 2 futures") {
    ...
    Async.blocking:
      ...
      Future {
        sleep(200)
        touched += 1
      }.altWithCancel(Future { 10 }).awaitTry
      sleep(300)
  ...
  test("zip") {
    Async.blocking:
      ...
      assertEquals(fail.zip(succeed).awaitTry, Failure(error))
      assertEquals(succeed.zip(fail).awaitTry, Failure(error))
      assertEquals(fail.zip(fail1).awaitTry, Failure(error))
  }
  ...
  test("result wraps exceptions") {
    Async.blocking:
      for (i <- -5 to 5)
        ...
        assertEquals(Future { throw error }.awaitTry, Failure(error))
  }

  test("result wraps values") {
    Async.blocking:
      for (i <- -5 to 5)
        assertEquals(Future { i }.awaitTry, Success(i))
  }
  ...
  test("future collector") {
    Async.blocking:
      ...
      var sum = 0
      for i <- range do sum += collector.results.read().get.await
