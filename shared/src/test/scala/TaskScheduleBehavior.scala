import gears.async.{Async, Future, Task, TaskSchedule, given}
...
class TaskScheduleBehavior extends munit.FunSuite {
  ...
  test("UntilSuccess 150ms") {
    ...
    Async.blocking:
      ...
      val ret = t.schedule(TaskSchedule.RepeatUntilSuccess(150)).run.awaitTry
  ...
  test("UntilFailure 150ms") {
    ...
    Async.blocking:
      ...
      val ret = t.schedule(TaskSchedule.RepeatUntilFailure(150)).run.awaitTry
